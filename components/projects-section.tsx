...
export function ProjectsSection() {
  ...
  // Default the section open, but let the user collapse it.
        ...
        <div className="flex flex-col pt-0.5">
          {projects.map((project) => {
            const chats = project.chatIds
              .map((id) => recentChats.find((c) => c.id === id))
...
function ProjectGroup({
  ...
  // Each project group remembers its own open/closed state.
  // We key the local state on the project id so it's independent
  // across projects.
  ...
  return (
    <div className="flex flex-col">
      <button
        ...
        className="group flex h-7 items-center gap-1.5 rounded-md px-2 text-[12.5px] text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
      >
        {expanded ? (
      ...
      {expanded && (
        <div className="ml-3 flex flex-col border-l border-sidebar-border/40 pl-1.5">
          {chats.length === 0 ? (
            ...
            chats.map((chat) => (
              <Link
                ...
                href={`/chat/${chat.id}`}
                className={cn(
                  'flex h-7 items-center gap-1.5 rounded-md px-2 text-[12.5px] text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground',
